...
java(const struct parser_param *param)
{
	...
	*classname = *completename = 0;
	...
	if ((T = tokenizer_open(param->gpath, NULL, NULL)) == NULL)
		die("'%s' cannot open.", param->gpath->path);
	while ((c = T->op->nexttoken(interested, java_reserved_word)) != EOF) {
		switch (c) {
		case SYMBOL:					/* symbol */
			for (; c == SYMBOL && T->op->peekchar(1) == '.'; c = T->op->nexttoken(interested, java_reserved_word)) {
			...
			if (startclass || startthrows) {
				...
				if (level == stack[classlevel].level && !startequal)
					/* ignore constructor */
					if (strcmp(stack[classlevel].classname, T->token))
		...
		case '}':
			if (--level < 0) {
				if (param->flags & PARSER_WARNING)
					warning("missing left '{' (at %d).", T->lineno); /* } */
				level = 0;
			}
		...
		case JAVA_INTERFACE:
		case JAVA_ENUM:
			if ((c = T->op->nexttoken(interested, java_reserved_word)) == SYMBOL) {
		...
		case JAVA_INSTANCEOF:
			while ((c = T->op->nexttoken(interested, java_reserved_word)) == SYMBOL && T->op->peekchar(1) == '.')
				PUT(PARSER_REF_SYM, T->token, T->lineno, T->sp);
			if (c == SYMBOL)
				PUT(PARSER_REF_SYM, T->token, T->lineno, T->sp);
			break;
		...
		case JAVA_SHORT:
		case JAVA_VOID:
			if (T->op->peekchar(1) == '.' && (c = T->op->nexttoken(interested, java_reserved_word)) != JAVA_CLASS)
				T->op->pushbacktoken();
			break;
                case '@':
			/* skip through annotations */
			if (T->op->nexttoken(interested, java_reserved_word) == SYMBOL) {
				if (T->op->peekchar(0) == '(') {
					int paren_count = 0;
					int in_annot = 1;

					while (in_annot) {
						c = T->op->nexttoken("()", java_reserved_word);
						if (c == '(') {
							++paren_count;
						} else if (c == ')') {
							--paren_count;
